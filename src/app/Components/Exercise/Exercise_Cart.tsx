...
const Exercise_Cart = ({ exercise_data }: props) => {
  ...
  return (
    <Link href={`/Cart_Details/${id}`}>
      <div className="w-full overflow-hidden rounded-xl border border-[#2A2D35] bg-[#17191F] shadow-sm">
        {/* Image */}
        ...
        <div className="p-4">
          {/* Muscle Groups */}
          <div className="[var(--font-inter)] flex flex-wrap gap-2 font-bold text-[#000000]">
            {muscleGroups.map((type: string, ind: number) => (
              <h1
                className="rounded-2xl border border-box bg-[#C2F800] px-3 py-1 text-[10px] uppercase"
                key={ind}
          ...
          {/* Exercise Name */}
          <div className="mt-3 text-sm sm:text-[15px] font-bold uppercase text-white">
            {name}
          </div>
